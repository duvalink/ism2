...
def create_pdf(cotizaciones):
 # Datos de la empresa
    ...
    # Ajusta width y height según las dimensiones deseadas
    ...
    direccion_empresa = Paragraph(
        direccion_empresa, style=minimal_leading_style)
    ...
    company_info_table = Table([
        [logo, '', [[nombre_empresa], [rfc_empresa], [
            direccion_empresa], [telefono_empresa]], '', logo]
    ...
    contactos = cotizaciones[0]['contactos']
    print(contactos)
    contacto_str = ", ".join([contacto['contacto'] for contacto in contactos])
    print(contacto_str)
    ...
    data_header = [
        ...
        ["Atencion:", contacto_str, "", ""],
    ]
    # Ajusta el tamaño de las columnas de espacio si es necesario
    header_table = Table(data_header, colWidths=[60, 280, 130, 100])
    ...
    data_totals = [
        ["MATERIALES", "$ {:,.2f}".format(
            materiales), "", "", "", "SUBTOTAL", "$ {:,.2f}".format(subtotal)],
        ["MANO DE OBRA", "$ {:,.2f}".format(
            mano_obra), "", "", "", "IVA", "$ {:,.2f}".format(iva)],
        ["", "", "", "", "", "TOTAL", "$ {:,.2f}".format(total)]
    ]
    # Ajusta el tamaño de las columnas de espacio si es necesario
    cols_data_totals = Table(data_totals, colWidths=[100, 30, 100, 50, 100])
    ...
    flowables = [company_info_table, spacer]
    ...
    header_table_width, header_table_height = header_table.wrap(
        doc.width, doc.height)
    table_width, table_height = table.wrap(doc.width, doc.height)
    table_totals_width, table_totals_height = table_totals.wrap(
        doc.width, doc.height)

    # Calcular espacio disponible en la página actual
    remaining_space = doc.height - \
        (header_table_height + spacer.height + table_height) - 75
